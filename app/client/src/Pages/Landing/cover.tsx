...
export default function Cover() {
    ...
    const Links = navLinks.map(navLink => {
        return <NavLink
            key={navLink.value}
            className={`w-24 ${(isActive: boolean) => isActive ? 'active' : ''}`}
            to={navLink.link}>
                {navLink.value}
        </NavLink>
    })
    ...
    return (
        <div id="home" className="cover flex flex-col w-full h-full relative">
            ...
            <section className="h-full flex justify-center items-center relative">
                ...
                <div className="content text-center flex flex-col justify-center items-center gap-8 px-28 z-10">
                    <h1>Elevate Your Ping Pong Game:<br></br>Master the Art of Digital Table Tennis like Never Before!</h1>
