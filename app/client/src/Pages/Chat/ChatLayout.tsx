...
function Navi () {
    ...
    return (
        <nav className='flex justify-between'>
            <NavLink  
            className={`nav_link flex justify-center items-center ${(isActive:boolean) => isActive ? 'active' : ''} flex`} 
            to="/chat" end>
                Messages
                {counter !== 0 && <span className=' rounded-lg block bg-pink-500 px-2 h-5 text-sm'>{counter}</span>}
            </NavLink>
            <NavLink 
            className={`nav_link flex justify-center items-center ${(isActive:boolean) => isActive ? 'active' : ''}`}  
            to="/chat/rooms">
                Channels
            </NavLink>
        </nav>
